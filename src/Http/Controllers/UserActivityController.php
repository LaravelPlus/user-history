...
final class UserActivityController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $recentActivitiesData = $recentActivities->map(fn ($activity) => [
            'id' => $activity->id,
    ...
    public function activities(Request $request): View
    {
        ...
        $activitiesData = $activities->getCollection()->map(fn ($activity) => [
            'id' => $activity->id,
            'user_id' => $activity->user_id,
            'user_name' => $activity->user ? $activity->user->name : 'Unknown User',
            'action' => $activity->action,
    ...
    public function apiStats(Request $request): JsonResponse
    {
        ...
        $activeUsersQuery = UserActivity::where('created_at', '>=', now()->subDays(7));
        if ($userId) {
        ...
        // Active users today
        $activeTodayQuery = UserActivity::where('created_at', '>=', now()->startOfDay());
        if ($userId) {
    ...
    public function timelineChartData(Request $request): JsonResponse
    {
        ...
        $query = UserActivity::selectRaw('DATE(created_at) as date, COUNT(*) as count')
    ...
    public function distributionChartData(Request $request): JsonResponse
    {
        ...
        $query = UserActivity::selectRaw('action, COUNT(*) as count')
            ->where('created_at', '>=', now()->subDays($days));
    ...
    public function users(Request $request): View
    {
        $users = \App\Models\User::select('id', 'name', 'email', 'created_at')
            ...
            ->map(function ($user) {
                ...
                $weeklyActivity = UserActivity::selectRaw('DATE(created_at) as date, COUNT(*) as count')
    ...
    public function userProfile(Request $request, $userId): View
    {
        ...
        $timelineData = UserActivity::selectRaw('DATE(created_at) as date, COUNT(*) as count')
    ...
    {
        $mostActiveDay = UserActivity::selectRaw('DATE(created_at) as date, COUNT(*) as count')
            ->where('user_id', $userId)
            ->groupBy('date')
            ->orderBy('count', 'desc')
            ->first();
